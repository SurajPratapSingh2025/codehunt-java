...
//		Session session=sf.openSession();							//session
		StatelessSession session = sf.openStatelessSession();		//stateless session
...
//		}
		...
		if(employee != null) {
			employee.setEsalary(7770);						//Not apply Dirty Checking
			session.update(employee);
			System.out.println("Data Updated");
			transaction.commit();
		}
...
//		session.save(employee);
		session.insert(employee);				//in stateless, we have insert() method
